...
class TransformerBlock(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_heads, dropout_rate=0.1):
        ...
        self.feed_forward = nn.Sequential(          # Feed Forward Neural Network
            nn.Linear(input_dim, hidden_dim),
            nn.ReLU(),                              # ReLU函数将负值变为0
            nn.Linear(hidden_dim, input_dim)
        )
        self.norm1 = nn.LayerNorm(input_dim)        # 歸一化
        self.norm2 = nn.LayerNorm(input_dim)
        self.dropout = nn.Dropout(dropout_rate)     # 防止过拟合
        
    def forward(self, x):
        attention_output, _ = self.attention(x, x, x) # Query Key Value
        x = x + self.dropout(attention_output)        # 将注意力输出添加到输入 x 中
        x = self.norm1(x)
        
        feed_forward_output = self.feed_forward(x)
        x = x + self.dropout(feed_forward_output)     # 引入一些非線性轉變，並且可以捕獲位置特定的特徵
        x = self.norm2(x)
...
if __name__ == "__main__":
    ...
    # 保存模型參數
    torch.save(policy_net.state_dict(), "ppo_training_results/policy_cnn_net.pth")
    torch.save(value_net.state_dict(), "ppo_training_results/value_cnn_net.pth")
